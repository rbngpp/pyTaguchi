...
class Taguchi():
    ...
    @property
    def FACTORS(self):
        return size(self.variables)

    def run(self, randomize=False, plot=False):
    ...
    def make_dummy(self, idx):
        var_dummy = {
            "name": f"Variable dummy.{idx}",
            "values": list(range(self.LEVELS))
        }
        self.n_dummy += 1
        return var_dummy

    def pad_dummy_vars(self, limit_factors):
        assert self.FACTORS <= limit_factors
        if self.FACTORS == limit_factors:
            return
        for idx in range(self.FACTORS, limit_factors):
            self.add(self.make_dummy(1 + idx))
        return

    def generate_design(self):
        if self.FACTORS <= 3 and self.LEVELS == 2:
            self.pad_dummy_vars(3)
            self.design = 'L4'
            self.OBSERVATIONS = 4
            breakpoint()
            self.design_L4()
        elif self.FACTORS <= 4 and self.LEVELS == 3:
            self.pad_dummy_vars(4)
            self.design = 'L9'
            self.OBSERVATIONS = 9
            self.design_L9()
        elif self.FACTORS <= 5 and self.LEVELS == 4:
            self.pad_dummy_vars(5)
            self.design = 'L16b'
            self.OBSERVATIONS = 16
            self.design_L16b()
        elif self.FACTORS <= 7 and self.LEVELS == 2:
            self.pad_dummy_vars(7)
            self.design = 'L8'
            self.OBSERVATIONS = 8
            self.design_L8()
        elif self.FACTORS <= 11 and self.LEVELS == 2:
            self.pad_dummy_vars(11)
            self.design = "L12"
    ...
    def randomize_runs(self):
        self.matrix = self.matrix[random.choice(self.matrix.shape[0],
                                                self.matrix.shape[0],
                                                replace=False)]
